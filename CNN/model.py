...
class Model(object):
    ...
    def inference(self, images, keep_prob):#The input shape is 250*250*1
        ...
        with tf.variable_scope('deconv6') as scope:
            ...
            reshape = tf.reshape(preactivation, shape=[-1,250,250])           
    ...
    def loss(self, y_pre, nandata, ys):
        with tf.variable_scope('loss') as scope:
            nanforpre = y_pre*nandata
            cross_entropy = tf.cast(tf.subtract(nanforpre,ys),tf.float32)
    ...
    def accuracy(self, logits, nandata, ys):
        with tf.variable_scope('accuracy') as scope:
            nanforlogits = logits*nandata
            accuracy = tf.reduce_mean(tf.cast(tf.abs(tf.subtract(nanforlogits,ys)),tf.float32),
                                      name=scope.name)
...
def get_data(modelname):
    ...
    prep = np.load('val_data/'+modelname+'/SDF.npy')
    nanfortrain = np.load('val_data/'+modelname+'/mask.npy')
    label = np.load('val_data/'+modelname+'/Label_T.npy')
    
    return prep, nanfortrain, label

def get_US(modelname):
    U = np.load('val_data/'+modelname+'/UT.npy')

    S = np.load('val_data/'+modelname+'/ST.npy')
